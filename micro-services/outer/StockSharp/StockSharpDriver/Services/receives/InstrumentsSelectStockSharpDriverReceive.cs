...
public class InstrumentsSelectStockSharpDriverReceive(IStockSharpDriverService ssRepo)
    : IMQTTReceive<TPaginationRequestStandardModel<InstrumentsRequestModel>, TPaginationResponseModel<InstrumentTradeStockSharpModel>>
{
    /// <inheritdoc/>
    public static string QueueName => GlobalStaticConstantsTransmission.TransmissionQueues.InstrumentsSelectStockSharpReceive;

    /// <inheritdoc/>
    public async Task<TPaginationResponseModel<InstrumentTradeStockSharpModel>> ResponseHandleActionAsync(TPaginationRequestStandardModel<InstrumentsRequestModel> req, CancellationToken token = default)
    {
        if (req is null)
