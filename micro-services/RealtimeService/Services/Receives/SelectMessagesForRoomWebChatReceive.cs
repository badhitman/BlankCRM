...
/// <summary>
/// SelectMessagesForRoomWebChat
/// </summary>
public class SelectMessagesForRoomWebChatReceive(IWebChatService webChatRepo)
    : IResponseReceive<TPaginationRequestAuthModel<SelectMessagesForWebChatRoomRequestModel>?, TPaginationResponseStandardModel<MessageWebChatModelDB>?>
{
    /// <inheritdoc/>
    public static string QueueName => GlobalStaticConstantsTransmission.TransmissionQueues.SelectMessagesForRoomWebChatReceive;

    /// <inheritdoc/>
    public async Task<TPaginationResponseStandardModel<MessageWebChatModelDB>?> ResponseHandleActionAsync(TPaginationRequestAuthModel<SelectMessagesForWebChatRoomRequestModel>? req, CancellationToken token = default)
    {
        ArgumentNullException.ThrowIfNull(req);
        return await webChatRepo.SelectMessagesForRoomWebChatAsync(req, token);
    }
}
