    ...
    IOptions<ConstructorConfigModel> _conf) : IConstructorService
{
    ...
    /// <inheritdoc/>
    public async Task<ResponseBaseModel> SetDoneSessionDocumentDataAsync(string token_session, CancellationToken cancellationToken = default)
    {
        using ConstructorContext context_forms = await mainDbFactory.CreateDbContextAsync(cancellationToken);
        ...
        TResponseModel<UserInfoModel[]> userRest = await identityRepo.GetUsersOfIdentityAsync([sq.AuthorUser], cancellationToken);
    ...
    public async Task<TResponseModel<SessionOfDocumentDataModelDB?>> SetValueFieldSessionDocumentDataAsync(SetValueFieldDocumentDataModel req, CancellationToken cancellationToken = default)
    {
        ...
        if (existing_value is null)
        {
            if (req.FieldValue is null)
                return await GetSessionDocumentAsync(new() { SessionId = req.SessionId }, cancellationToken);
    ...
    public async Task<ResponseBaseModel> DeleteValuesFieldsByGroupSessionDocumentDataByRowNumAsync(ValueFieldSessionDocumentDataBaseModel req, CancellationToken cancellationToken = default)
    {
        ...
        if (values_for_delete.Length > 0)
        {
            ...
            get_s = await GetSessionDocumentAsync(new() { SessionId = req.SessionId }, cancellationToken);
            uint i = 0;
    ...
    public async Task<ResponseBaseModel> UpMoveElementOfDirectoryAsync(TAuthRequestModel<int> req, CancellationToken cancellationToken = default)
    {
        ...
        context_forms.UpdateRange([element_db.Parent!.Elements![i], element_db.Parent!.Elements![i - 1]]);
        await context_forms.SaveChangesAsync(cancellationToken);
    ...
    public async Task<TResponseModel<FormToTabJoinConstructorModelDB?>> GetTabDocumentSchemeJoinFormAsync(int tab_document_scheme_join_form_id, CancellationToken cancellationToken = default)
    {
        ...
        if (res.Response is null)
            ...
            res.Response.Tab.JoinsForms = [.. res.Response.Tab.JoinsForms.OrderBy(x => x.SortIndex)];
