...
public class FirebaseTransmissionRabbit([FromKeyedServices(nameof(RabbitClient))] IMQStandardClientRPC rabbitClient) : IFirebaseService, IFirebaseServiceTransmission
{
    ...
    public async Task<TResponseModel<SendFirebaseMessageResultModel>> SendFirebaseNotificationAsync(TAuthRequestStandardModel<SendFirebaseMessageRequestModel> req, CancellationToken token = default)
        => await rabbitClient.MqRemoteCallAsync<TResponseModel<SendFirebaseMessageResultModel>>(GlobalStaticConstantsTransmission.TransmissionQueues.SendFirebaseNotificationReceive, req, token: token) ?? new();
}