...
public partial class GeneratorClientViewComponent : FieldComponentBaseModel
{
    ...
    string? FieldValue => SessionOfDocumentData?.DataSessionValues?.FirstOrDefault(x => x.Name.Equals(Field.Name, StringComparison.OrdinalIgnoreCase) && x.RowNum == GroupByRowNum)?.Value;
    ...
    protected override void OnInitialized()
    {
        ...
        if (SessionOfDocumentData is not null)
        {
            TResponseModel<string[]> res_elements = _gen.GetListElements(Field, SessionOfDocumentData, PageJoinForm, GroupByRowNum);
