...
docker exec -t srv-postgres-1 pg_dump -c -U dev FilesIndexingContext --format=p --encoding=UTF-8 --inserts | gzip > /srv/tmp/dumps/dump_FilesIndexing_`date +%Y-%m-%d"_"%H_%M_%S`.sql.gz
