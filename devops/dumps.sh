...
docker exec -t srv_postgres_1 pg_dump -c -U dev CommerceContext --format=p --encoding=UTF-8 --inserts | gzip > /srv/tmp/dumps/dump_Commerce_`date +%Y-%m-%d"_"%H_%M_%S`.sql.gz
docker exec -t srv_postgres_1 pg_dump -c -U dev HelpDeskContext --format=p --encoding=UTF-8 --inserts | gzip > /srv/tmp/dumps/dump_HelpDesk_`date +%Y-%m-%d"_"%H_%M_%S`.sql.gz
docker exec -t srv_postgres_1 pg_dump -c -U dev StorageContext --format=p --encoding=UTF-8 --inserts | gzip > /srv/tmp/dumps/dump_Storage_`date +%Y-%m-%d"_"%H_%M_%S`.sql.gz
