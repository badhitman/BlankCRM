server {
    listen 80;
    server_name realtime.iq-s.pro www.realtime.iq-s.pro;
    # return 301 https://$host$request_uri;
	large_client_header_buffers 4 128k;
	
	access_log /var/log/nginx/nginx.http.realtime.iq-s.pro.access.log;
	error_log /var/log/nginx/nginx.http.realtime.iq-s.pro.error.log;
...
server {
	...
	access_log /var/log/nginx/nginx.https.realtime.iq-s.pro.access.log;
	error_log /var/log/nginx/nginx.https.realtime.iq-s.pro.error.log;
 
 	add_header X-Content-Type-Options "nosniff";
 	add_header 'Access-Control-Allow-Headers' 'token-access,DNT,X-CustomHeader,Keep-Alive,User-Agent,X-Requested-With,If-Modified-Since,Cache-Control,Content-Type' always;
 
     location / {
 		proxy_pass https://localhost:1983;
 		proxy_http_version 1.1;
 		proxy_set_header Upgrade $http_upgrade;
 		proxy_set_header Connection $http_connection;
 		proxy_set_header Host $host;
