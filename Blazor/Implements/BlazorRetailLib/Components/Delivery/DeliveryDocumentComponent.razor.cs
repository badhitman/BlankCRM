...
public partial class DeliveryDocumentComponent : BlazorBusyComponentBaseAuthModel
{
    ...
    bool CannotSave
    {
        get
        {
            if (IsBusyProgress || currentDoc is null || editDoc is null || editDoc.DeliveryTypeId <= 0)
                return true;
    ...
    async Task SaveDoc()
    {
        ...
        if (editDoc.Id <= 0)
        {
            TResponseModel<int> res = await RetailRepo.CreateDeliveryDocumentRetailAsync(new()
            {
        ...
        {
            TResponseModel<Guid?> res = await RetailRepo.UpdateDeliveryDocumentRetailAsync(new() { SenderActionUserId = CurrentUserSession.UserId, Payload = editDoc });
            SnackBarRepo.ShowMessagesResponse(res.Messages);
            if (res.Success())
            {
