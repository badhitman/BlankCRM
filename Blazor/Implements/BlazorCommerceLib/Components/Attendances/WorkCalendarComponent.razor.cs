...
namespace BlazorCommerceLib.Components.Attendances;
...
public partial class WorkCalendarComponent : BlazorBusyComponentBaseAuthModel
{
    ...
    public async Task Reload(OfferModelDB? selectedOffer)
    {
        ...
        await SetBusyAsync();
        TResponseModel<TPaginationResponseStandardModel<CalendarScheduleModelDB>> res = await CommerceRepo.CalendarsSchedulesSelectAsync(new() { Payload = req, SenderActionUserId = CurrentUserSession.UserId });
