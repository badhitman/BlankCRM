...
namespace BlazorHelpdeskLib.Components.issue;
...
public partial class StatusIssueComponent : IssueWrapBaseModel
{
    ...
    List<StatusesDocumentsEnum> Steps()
    {
        ...
        if (CurrentUserSession?.IsAdmin == true || CurrentUserSession?.UserId == Issue.ExecutorIdentityUserId || CurrentUserSession?.Roles?.Contains(GlobalStaticConstantsRoles.Roles.HelpDeskManager) == true)
